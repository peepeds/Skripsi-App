  ...
  export function InboxDetailView({
    ...
    const renderObjectAsList = (obj, title) => {
      ...
      return (
        <div>
          <h3 className="mb-4 font-plus-jakarta text-xl font-bold text-slate-900">{title}</h3>
          <div className="space-y-3">
            {Object.entries(obj).map(([key, value]) => (
              <div
                key={key}
                className="grid gap-1 rounded-xl border border-slate-200 bg-slate-50/50 p-3 md:grid-cols-[220px_1fr] md:items-start md:gap-3"
              >
                <span className="font-inter text-sm capitalize text-slate-500">
                  {formatKeyLabel(key)}:
                </span>
        ...
        <Card className="rounded-2xl border-slate-200 py-0 shadow-sm">
          <CardHeader className="border-b border-slate-200 py-6">
            <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
              <div>
                <CardTitle className="font-plus-jakarta text-3xl font-bold leading-tight text-slate-900">
                  Inbox Details
                </CardTitle>
          ...
          <CardContent className="space-y-8 py-6">
            ...
            <div>
              <h3 className="mb-4 font-plus-jakarta text-xl font-bold text-slate-900">Activity Log</h3>
              {auditLoading ? (
                ...
                <div className="space-y-3">
                  {timeline.map((log, index) => (
                    <div
                      ...
                      className={`rounded-xl border-l-4 p-3 ${log.toneClass}`}
                    >
                ...
                <p className="py-4 text-center font-inter text-sm text-muted-foreground">
